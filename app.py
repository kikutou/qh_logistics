    ...
    if request.method == 'POST':
        ...
        if (item == 0 or (item and ( item['jp_name'] == None or item['unit_price'] == None ))):
            ...
            try:
                ...
                # 商品の日本語名前
                itemNameJp = itemTable.find('div','itemBg').find('div','itemInfo').find('p','itemnameN').contents[0]

                # 商品の参照単価
                price = itemTable.find('div','itemBg').find('div','itemInfo').find('div','clearfix').find('div','itemDbox')\
                    .find('p','price').find('span','yen').string

                if price:
                    price = price[1:]

                # 商品の備考（分類）
                memo = itemTable.find('div','itemBg').find('div','itemInfo').find('div','clearfix').find('div','itemDbox')\
                    .find('div','itemDetail').find('p','cate').string
...
def itemAddSuccess():
    ...
    if inboxes:
        ...
        inboxes_tmp = Inbox(app).getInboxDatas({'status': 2, 'staff_id': session.get('staff_id')})
        inbox = inboxes_tmp[0]
        customerId = inbox['customer_id']
        customer = Customer(app).getCustomer(int(customerId))
        if not customer:
...
def itemEdit():
    ...
    if request.method == 'POST':
        ...
        outbox_rel_id = int(request.form['outbox_rel_id'])
        outbox_id = int(request.form['outbox_id'])
        ...
        item_num = request.form['number']
        if int(item_num) == 0:
            result = RelOutItem(app).deleteRelOutItem(outbox_rel_id)
        else:
            result = RelOutItem(app).saveRelOutItem(outbox_rel_id, {'item_num': item_num})
        # if result == 0:
        #     error = '数量更新失败'

        Item(app).saveItem(item_id, data)

        return redirect('/outboxInfo?id=' + str(outbox_id) + '&source=' + source)

    source = request.args.get('source')
    item_id = int(request.args.get('item_id'))
    item = Item(app).getItem(item_id)
    outbox_rel_id = int(request.args.get('outbox_rel_id'))
    outbox = RelOutItem(app).getRelOutItem({'id':outbox_rel_id, 'item_id': item_id})
    if outbox != 0:
        item_num = outbox['item_num']
    else:
        item_num = 0
    madeInInfos = MadeInInfo(app).getMadeInInfos(None)
    # print(item)
...
def outboxInfo():
    ...
    for key,relOutItem in relOutItems.items():
        ...
        memo = item['memo']
        country_of_origin_id = item['country_of_origin']
        country_of_origin = MadeInInfo(app).getMadeInInfo(int(country_of_origin_id))['made_in_country']
        item_image_pic = item['item_image_pic']

        data = {
            'id': itemId,
            'outboxRelId': outboxRelId,
            'item_num': itemNum,
            'jp_name': itemNameJp,
            'en_name': itemNameEn,
            'cn_name': itemNameCn,
...
def adminOutbox():
    ...
    for key,record in outboxData.items():
        ...
        if not record['delivery_no']:
            record['delevery_no'] = '还未填写'
...
def adminOutboxEdit():
    ...
    if request.method == 'POST':
        ...
        if result:
            flash('出库箱更新成功')
            if action:
                if action == 'adminOutbox':
                    return redirect('/%s' % (action))
                else:
                    return redirect('/%s?id=%s' % (action, outboxId))
            else:
