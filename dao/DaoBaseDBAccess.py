...
class DaoBase(object):
    ...
    def __init__(self):
        ...
        mysql = MySQL()
        app.config['MYSQL_DATABASE_USER'] = configure.MYSQL_DATABASE_USER
        app.config['MYSQL_DATABASE_PASSWORD'] = configure.MYSQL_DATABASE_PASSWORD
        app.config['MYSQL_DATABASE_DB'] = configure.MYSQL_DATABASE_DB
        app.config['MYSQL_DATABASE_HOST'] = configure.MYSQL_DATABASE_HOST
        mysql.init_app(app)
    ...
    def deleteRecordOne(self, tableName, where):
        if tableName is None:
            return 0
        if where is None:
            return 0
        else:
            deleteString = "DELETE FROM " + str(tableName)
            ...
            try:
                ...
                pass
            if isinstance(where, int):
                deleteString += " WHERE ID=" + str(where) + ";"

        result = self.conn.cursor().execute(deleteString)
